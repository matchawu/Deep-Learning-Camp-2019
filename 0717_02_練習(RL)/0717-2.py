...
class Agent():
    ...
    def sampleAction(self, s):
        ...
        action = np.random.choice(2,p=self.model.predict(s)[0] )
...
#        print("pig"%s_buffer[:buffer_counter])
#        ss = np.concatenate((self.s_batch, s), axis=0)
#        aa = np.concatenate((self.a_batch, a), axis=0)
#        rr = np.concatenate((self.r_batch, r), axis=0)
#        r_mean = np.mean(r)
#        r-=r_mean
#        self.s_batch[self.n1] = s
#        self.a_batch[self.n1] = a
#        self.r_batch[self.n1] = r
#        print(r)
#        self.n1+=1
        #print("aa"%self.s_batch)
        #print(s)
#        utilsToos.processReward(r)
...
#        self.model.predict(self.s)
        
        prob_predict = self.model.predict(self.s_batch) * self.a_batch
        self.model.fit(self.s_batch,prob_predict,sample_weight=self.r_batch)
        self.n1=0
        return
