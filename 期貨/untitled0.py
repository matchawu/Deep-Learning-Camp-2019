...
def CLOSE_PRICE(close,get_df,year,month,day,hour,minute,second):
    ...
    am = datetime.datetime(year,month,day,8,45,0)
    pm = datetime.datetime(year,month,day,13,44,0)
    
    if date < am:
        #get yesterday 13:44 close price
        idx = close.index[close['DT'] == am]
        idx = idx[0]-1
        price = close.iloc[idx]['Close']
        get_df['close'] = price
        
    elif date > pm:
        #get next day 8:45 close price
        idx = close.index[close['DT'] == pm]
        idx = idx[0]+1
        price = close.iloc[idx]['Close']
        get_df['close'] = price
    else:
...
def SPOINT(df,year,month,day,hour,minute,second):
    ...
    get_df = CLOSE_PRICE(close,get_df,year,month,day,hour,minute,second)
    
    del get_df['F']
    del get_df['I']
    del get_df['R']
    del get_df['OSF_ORDER_QNTY']
    del get_df['DT']
    print(date)
...
#%%
test = df_17_1
yy = SPOINT(test,2017,1,18,13,40,0)
#zz = PERIOD(test,2018,1,17,11,30,0,2018,1,18,11,31,0)
